...
#实例化一个变量做测试,并且传入一个字符串,作为变量,这个变量被__init__()方法调用,用来初始化这个对象
classtest = ClassTest("val3 value")


# 在类中,普通定义一个方法,也成实例方法,只有实例化一个对象之后,才能有这个实力来调用
# 实例方法可以直接访问实例的实例变量(self.val2,self.val3,self.val4),所有的成员变量,都可以作为他的参数来调用
# 顾名思义,实例方法,只能被一个实例调用,定义的时候,第一个参数必须是self,即第一个参数self,指的是这个实例自己
classtest.normalmethod("this is a dd value")  #正确调用
# ClassTest.normalmethod("this is a test")      #错误调用

#用@staticmethod装饰器来定义的方法,成为静态方法,静态方法无法访问类中的一切变量,你可以理解为这个是独立存在的。
# 静态方法,可以被实例调用,也可以被类调用,说是调用,不如说: 静态方法前面的实例,还有类,只是指定这个方法的作用域
...
# 注意:这里的名词和使用习惯,我是采用和ruby对比的方法记忆的,为了不让两种语言在我脑子中乱掉,尽量采用相似的记忆方法。
